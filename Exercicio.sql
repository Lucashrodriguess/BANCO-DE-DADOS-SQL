...
/* exercicio curos banco de dados aula 33*/ 
...
/*SELECT sexo, COUNT(*) FROM funcionarios GROUP by sexo; // separa o grupo por sexo e conta a quantidade de cada*/  
/*SELECT sexo FROM funcionarios GROUP by sexo; // separa o grupo por sexo mais nao faz a contagem
SELECT departamento FROM funcionarios GROUP by ; // separa o grupo por departamento mais nao faz a contagem*/ 
...
from funcionarios 
GROUP by departamento order by 1; /*verificando quantos funcionarios de cada area tem.
 
/* select count(*), departamento from funcionarios group by departamento;  mesma forma do de cima.
...
SELECT sexo, COUNT(*) 
FROM funcionarios 
GROUP by sexo; /* quantidade de pessoas sexo masculino e feminino */ 


SELECT departamento, COUNT(*) 
FROM funcionarios 
GROUP by departamento; /* quantidade de pessoas em cada grupo de trabalho


/* verificando quantos trabalha em filmes e em lar*/ 

select sexo, departamento, count(*) 
from funcionarios 
...
/* Traga a quantidade de funcionarios do sexo masculino que trabalham no setor industrial  */
select sexo, departamento, count(*)
from funcionarios
WHERE sexo ='Masculino' and departamento ='Industrial';

/* traga nome, sexo e departamento de todos os funcionarios e conte a quantidade de funcionarios em cada setor. */
select nome, sexo, departamento, count(*)
from funcionarios
group by nome, sexo, departamento;


/* traga nome, sexo e departamento dos funcionarios quem tem como nome Allen 


QUANTOS FUNCIONARIOS CHAMADOS ALLEN TEM EM CADA DEPARTAMENTO*/
select nome, sexo, departamento, count(*)
from funcionarios
where nome = 'Allen'
group BY nome,sexo,departamento;

/* traga nome e departamento de todos os funcionarios masculinos e conte a quantidade de funcionarios em cada departamento. */

SELECT nome, departamento, count(*)
from  funcionarios  
where sexo ='Masculino'
group by nome, departamento;

/* quantodade de funcionarios que trabalaham em cada departameto */ 
select sexo, departamento, count(*)
from funcionarios
group by sexo, departamento order by 2; /* ordenando pelo departamento como ele foi o segundo item do select
