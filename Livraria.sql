...
select Nome_livro, estado, Nome_Autor 
from livros 
where sexo ='M';
...
select nome_Livro, Num_Paginas, Nome_Autor 
from livros 
where sexo = 'F';
...
select nome_autor, sexo, estado, editora, ano_Publicado 
from livros 
where ano_Publicado >= 2015 AND (num_paginas >=100 and sexo ='M');
...
SELECT COUNT(*) AS "Quantidade de Registros da Tab. Livros" 
FROM livros;
...
select editora, count(*) "Quantidade" 
from livros 
GROUP by editora;
